...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        down_9 = self.down_convolution_4(down_8)

        # up decoding
        up_1 = self.up_transpose_1(down_9)
        x = self.up_convolution_1(torch.cat([down_7, up_1]))
        up_2 = self.up_transpose_2(x)
        x = self.up_convolution_2(torch.cat([down_5, up_2]))
        up_3 = self.up_transpose_3(x)
        x = self.up_convolution_3(torch.cat([down_3, up_3]))
